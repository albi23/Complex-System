...
def task5(list_x: List[T], list_y: List[T]):
    ...
    """
    return list(set(list_x).intersection(list_y))
...
def task9():
    ...
    for c in input_str:
        histogram[c] += 1
    print({k: v for k, v in sorted(histogram.items(), key=lambda item: item[1])})
...
#     """
#     return set(text6.tokens).intersection(text7.tokens)
#
#
# def task19():
#     """
